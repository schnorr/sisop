...
- Falta de página em um thread bloqueia todos os demais
- Apenas um thread acessa o kernel por vez
...
- SO não conhece as threads internos; bloqueia o processo ao buscar página

#+latex: \vfill\pause

Ausência de preempção interna

- Sem interrupção de relógio, threads não podem ser preemptados
- Um thread que não cede a CPU voluntariamente impede todos os outros
