...
- P4 pode liberar sua instância de R2 → recurso vai para P3
...
1. Prevenção ou evitação (deadlock nunca ocorre)
...
3. Ignorar o problema (algoritmo do avestruz)
   - Abordagem adotada pelo Linux, Windows e maioria dos SOs
...
- Técnica de spooling: múltiplos processos geram saída simultaneamente
...
- Impor uma ordem numérica global a todos os tipos de recursos
...
Usado quando cada tipo de recurso possui apenas uma instância
...
O sistema deve manter o grafo e periodicamente invocar o algoritmo
- Custo: O(n²) sendo n o número de processos

* Detecção: Múltiplos Recursos
